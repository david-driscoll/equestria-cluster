...
var postgres = opClient.get


// Configuration from environment variables
var postgresHost = Environment.GetEnvironmentVariable("POSTGRES_HOST") ?? "postgres-rw.database.svc.cluster.local";
var postgresPort = Environment.GetEnvironmentVariable("POSTGRES_PORT") ?? "5432";
var postgresUser = Environment.GetEnvironmentVariable("POSTGRES_USER") ?? "postgres";
var postgresPassword = Environment.GetEnvironmentVariable("POSTGRES_PASSWORD") ?? throw new InvalidOperationException("POSTGRES_PASSWORD is required");
var s3Bucket = Environment.GetEnvironmentVariable("S3_BUCKET") ?? "equestria-postgres-backups";
var backblazeKeyId = Environment.GetEnvironmentVariable("AWS_ACCESS_KEY_ID") ?? throw new InvalidOperationException("AWS_ACCESS_KEY_ID is required");
var backblazeKey = Environment.GetEnvironmentVariable("AWS_SECRET_ACCESS_KEY") ?? throw new InvalidOperationException("AWS_SECRET_ACCESS_KEY is required");
...
var backblazeClient = new BackblazeClient(backblazeKeyId, backblazeKey);
await backblazeClient.Authorize();

// Get bucket info
var bucketsResponse = await backblazeClient.Buckets.GetList();
var bucket = bucketsResponse.FirstOrDefault(b => b.BucketName == s3Bucket);
if (bucket == null)
{
  throw new InvalidOperationException($"Bucket '{s3Bucket}' not found");
}

// Get list of databases
Console.WriteLine("Fetching list of databases...");
var databases = await GetDatabases(postgresHost, postgresPort, postgresUser, postgresPassword);
...
// Helper methods
async Task<List<string>> GetDatabases(string host, string port, string user, string password)
{
  var psi = new ProcessStartInfo
  {
    FileName = "psql",
    Arguments = $"-h {host} -p {port} -U {user} -d postgres -t -c \"SELECT datname FROM pg_database WHERE datistemplate = false AND datname NOT IN ('postgres', 'template0', 'template1');\"",
