...
var tagMap = new Dictionary<string, (ServiceKind Kind, Func<string, string> ServerName)>
{
    ...
    ["tag:backups"] = (ServiceKind.Pbs,     h => h),
};
...
var excludePorts = new Dictionary<string, Dictionary<ServiceKind, string[]>>
{
    // celestia's proxmox node doesn't expose SSH
    ["celestia"] = new() { [ServiceKind.Proxmox] = ["ssh"] },
};
...
var apiKey  = Environment.GetEnvironmentVariable("TAILSCALE_API_KEY");
...
foreach (var (server, kinds) in serverKinds.OrderBy(x => x.Key))
{
    ...
    foreach (var kind in kinds)
    {
        ...
        if (excludePorts.TryGetValue(server, out var kindExcludes) &&
            kindExcludes.TryGetValue(kind, out var excluded))
            ports.RemoveAll(p => excluded.Contains(p.Name));
    ...
    - name: blackbox_probes
      ...
      rules:
        ...
        - alert: BlackboxProbeFailing
          annotations:
            description: "Blackbox probe '{{ $labels.probe }}' has been failing."
        ...
        - alert: BlackboxProbeFailingCritical
          annotations:
            description: "Blackbox probe '{{ $labels.probe }}' is critically failing."
        ...
        - alert: BlackboxProbeHighLatency
          annotations:
            description: "Blackbox probe '{{ $labels.probe }}' p99 latency is {{ $value | humanizeDuration }}."
        ...
        - alert: BlackboxProbeSslCertificateExpiringSoon
          annotations:
            description: "SSL certificate for probe '{{ $labels.probe }}' expires in {{ $value | humanizeDuration }}."
        ...
        - alert: BlackboxProbeTimeout
          annotations:
            description: "Blackbox probe '{{ $labels.probe }}' is timing out."
