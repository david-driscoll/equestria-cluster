...
- volsync (local / remote / backblaze)
  ...
  - Usage: Apps that require replicated volumes include `components/volsync` (or `volsync`) and use the provided templates (PVC, ReplicationSource, ReplicationDestination, externalsecret). Many `Update.cs` scripts call these templates programmatically.
  - Examples: `kubernetes/components/volsync/replicationsource.yaml`, `kubernetes/components/volsync/pvc.yaml`, `kubernetes/apps/sgc/dns/adguard-home/Update.cs`.
...
- Behavior: The scripts read templates (e.g., `GetTemplate("kubernetes/components/volsync/replicationsource.yaml")`), generate resource YAML, write files, and (for `.sops.yaml`) re-encrypt using `sops -e -i` (see `kubernetes/components/common/Update.cs` for patterns).
