...
All internal services — including the Kubernetes cluster and external nodes (Proxmox hosts, dockge servers) — are accessible over the **Tailscale VPN mesh**. You must be connected to Tailscale to reach any of the endpoints listed below.

External nodes are exposed to the cluster via the Tailscale Operator's `ExternalName` services in `kubernetes/apps/tailscale-system/services/`. These are generated by `Update.cs` — do not edit them manually; run `task update` instead.

| Server | Role | Services on Tailnet |
|--------|------|---------------------|
| `celestia` | Proxmox hypervisor + dockge host | Dockge (`dockge-celestia`), Proxmox, PBS |
| `luna` | Proxmox hypervisor + dockge host | Dockge (`dockge-luna`), Proxmox, PBS |
| `skystar` | Proxmox hypervisor + dockge host | Dockge (`dockge-skystar`), Proxmox |
| `as` | Dockge host | Dockge (`dockge-as`) |
| `twilight-sparkle` | Proxmox hypervisor | Proxmox only |
| `alpha-site` | Proxmox hypervisor | Proxmox only |

Dockge instances are accessible at `https://dockge-<server>.${TAILSCALE_DOMAIN}` (e.g. `dockge-celestia.<tailnet>`).

# Troubleshooting Endpoints

When investigating issues across either the Kubernetes cluster or the dockge stack, query the Alertmanager API first to see all currently firing alerts:

```bash
curl -s https://alertmanager.driscoll.tech/api/v2/alerts | jq .
```

- **No authentication required** — the endpoint is open to clients on the Tailscale network
- **Scope**: covers alerts from both the Kubernetes cluster and all dockge nodes
- **Not internet-accessible** — only reachable over Tailscale; ensure you are connected before querying
- Useful fields: `labels.alertname`, `labels.namespace`, `labels.pod`, `annotations.summary`, `status.state`
